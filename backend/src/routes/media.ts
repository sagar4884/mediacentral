...
router.get('/', async (req, res) => {
  try {
    ...
    if (status) whereClause.keepStatus = String(status);
    if (source) whereClause.source = String(source);
    
    const media = await prisma.mediaCache.findMany({
      where: whereClause,
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const moviesResult = await prisma.mediaCache.aggregate({ _sum: { sizeOnDisk: true }, where: { source: 'Radarr' } });
    const showsResult = await prisma.mediaCache.aggregate({ _sum: { sizeOnDisk: true }, where: { source: 'Sonarr' } });
    ...
    const unraidUrl = await prisma.setting.findUnique({ where: { key: 'UnraidURL' } });
    const unraidKey = await prisma.setting.findUnique({ where: { key: 'UnraidKey' } });
    const radarrUrl = await prisma.setting.findUnique({ where: { key: 'RadarrURL' } });
    const radarrKey = await prisma.setting.findUnique({ where: { key: 'RadarrKey' } });
    const tautulliUrl = await prisma.setting.findUnique({ where: { key: 'TautulliURL' } });
    const tautulliKey = await prisma.setting.findUnique({ where: { key: 'TautulliKey' } });

    const storageProvider = await prisma.setting.findUnique({ where: { key: 'StorageProvider' } });
    ...
    if (provider === 'Unraid' && unraidUrl?.value && unraidKey?.value) {
      try {
        ...
        if (unraidRes.data?.data?.array?.capacity?.kilobytes) {
          ...
          if (totalSpace > 0) storageFetched = true;
        }
      } catch (e) {
    ...
    if (!storageFetched && radarrUrl?.value && radarrKey?.value) {
      try {
        const dsRes = await axios.get(`${radarrUrl.value}/api/v3/diskspace`, { headers: { 'X-Api-Key': radarrKey.value } });
    ...
    if (tautulliUrl?.value && tautulliKey?.value) {
      try {
        const tRes = await axios.get(`${tautulliUrl.value}/api/v2`, {
    ...
    const recent = await prisma.mediaCache.findMany({
      take: 5,
...
router.post('/manual-curate', async (req, res) => {
  try {
    ...
    const autoKeepWatched = await prisma.setting.findUnique({ where: { key: 'AutoKeepWatchedMedia' } });
    const autoKeepRequested = await prisma.setting.findUnique({ where: { key: 'AutoKeepRequestedMedia' } });
    const tautulliThresholdSetting = await prisma.setting.findUnique({ where: { key: 'TautulliShowWatchThreshold' } });
    const plexUrlSetting = await prisma.setting.findUnique({ where: { key: 'PlexURL' } });
    const plexTokenSetting = await prisma.setting.findUnique({ where: { key: 'PlexToken' } });
    ...
    let watchedCounts = new Map<string, number>();
    let plexShowsWatched = new Map<string, { viewed: number, total: number }>();
    ...
    if (isAutoKeepWatched) {
      if (urlSetting?.value && keySetting?.value) {
        try {
          ...
          const libsRes = await axios.get(`${url}/api/v2`, {
          ...
          for (const lib of libraries) {
            if (lib.section_type === 'movie' || lib.section_type === 'show') {
              const mediaRes = await axios.get(`${url}/api/v2`, {
      ...
      if (plexUrlSetting?.value && plexTokenSetting?.value) {
        try {
          const plexRes = await axios.get(`${plexUrlSetting.value}/library/sections`, {
          ...
          for (const section of sections) {
            if (section.type === 'show') {
              const sectionRes = await axios.get(`${plexUrlSetting.value}/library/sections/${section.key}/all`, {
                headers: { 'X-Plex-Token': plexTokenSetting.value, 'Accept': 'application/json' }
              });
    ...
    const waitingMedia = await prisma.mediaCache.findMany({
      ...
      if (isAutoKeepWatched) {
        ...
        if (media.source === 'Sonarr' && plexShowsWatched.has(normName)) {
          ...
          if (plexShowData) {
            ...
            if (showThreshold === 'full' && viewed >= total && total > 0) isWatched = true;
            else if (showThreshold === 'half' && (viewed / total) >= 0.5 && total > 0) isWatched = true;
            else if (showThreshold === 'any' && viewed > 0) isWatched = true;
            
            if (isWatched) keepReason = `Watched ${viewed}/${total} episodes on Plex`;
          }
        }
...
router.post('/:id/action', async (req, res) => {
  try {
    ...
    const media = await prisma.mediaCache.findUnique({ where: { id } });
    if (!media) return res.status(404).json({ error: 'Media not found' });

    // Update the media cache
    let newStatus = 'waiting';
    if (action === 'keep') newStatus = 'kept';
    if (action === 'delete') newStatus = 'marked_for_deletion';
    if (action === 'archive') newStatus = 'archive';
    if (action === 'wait') newStatus = 'waiting';
    
    const updateData: any = { keepStatus: newStatus, keepReason: reason };
...
router.post('/:id/instant-delete', async (req, res) => {
  try {
    const { id } = req.params;
    await actionService.executeInstantDelete(id, true);
...
router.get('/image', async (req, res) => {
  try {
    ...
    if (!url || typeof url !== 'string' || !source) return res.status(400).json({ error: 'Missing url or source' });

    let baseUrl = '';
    let apiKey = '';
    
    if (source === 'Radarr') {
      baseUrl = (await prisma.setting.findUnique({ where: { key: 'RadarrURL' } }))?.value || '';
      apiKey = (await prisma.setting.findUnique({ where: { key: 'RadarrKey' } }))?.value || '';
    } else if (source === 'Sonarr') {
      baseUrl = (await prisma.setting.findUnique({ where: { key: 'SonarrURL' } }))?.value || '';
      apiKey = (await prisma.setting.findUnique({ where: { key: 'SonarrKey' } }))?.value || '';
    }

    if (!baseUrl || !apiKey) return res.status(400).json({ error: 'Source config missing' });
...
router.get('/tmdb', async (req, res) => {
  try {
    ...
    const keySetting = await prisma.setting.findUnique({ where: { key: 'TMDBKey' } });
    if (!keySetting?.value) return res.status(400).json({ error: 'TMDB Key not configured in Settings' });
...
router.get('/tvdb', async (req, res) => {
  try {
    ...
    const keySetting = await prisma.setting.findUnique({ where: { key: 'TVDBKey' } });
    if (!keySetting?.value) return res.status(400).json({ error: 'TVDB Key not configured in Settings' });

    // 1. Login to get token
    const loginRes = await axios.post('https://api4.thetvdb.com/v4/login', { apikey: keySetting.value });
    const token = loginRes.data?.data?.token;
    if (!token) throw new Error("TVDB Login failed");

    // 2. Fetch extended series info (includes cast/characters)
    const seriesRes = await axios.get(`https://api4.thetvdb.com/v4/series/${id}/extended?meta=translations`, {
    ...
    const cast = characters.map((c: any) => ({
      id: c.id,
      name: c.personName,
      character: c.name,
...
router.post('/bulk-tag-kept', async (req, res) => {
  try {
    ...
    const items = await prisma.mediaCache.findMany({
    ...
    const urlSetting = await prisma.setting.findUnique({ where: { key: `${source}URL` } });
    const keySetting = await prisma.setting.findUnique({ where: { key: `${source}Key` } });
    ...
    let tagId = null;
    const tagsRes = await axios.get(`${baseUrl}/api/v3/tag`, { headers });
    const tags = tagsRes.data || [];
    const existingTag = tags.find((t: any) => t.label.toLowerCase() === 'ai-keep');
    
    if (existingTag) {
      tagId = existingTag.id;
    } else {
      const createRes = await axios.post(`${baseUrl}/api/v3/tag`, { label: 'ai-keep' }, { headers });
    ...
    if (sourceIds.length > 0) {
      if (source === 'Radarr') {
        await axios.put(`${baseUrl}/api/v3/movie/editor`, {
          movieIds: sourceIds,
          tags: [tagId],
          applyTags: 'add'
        }, { headers });
      } else {
        await axios.put(`${baseUrl}/api/v3/series/editor`, {
          seriesIds: sourceIds,
          tags: [tagId],
          applyTags: 'add'
        }, { headers });

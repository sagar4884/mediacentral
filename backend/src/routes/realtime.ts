...
router.get('/', async (req, res) => {
  try {
    ...
    const jellyseerrUrl = await prisma.setting.findUnique({ where: { key: 'JellyseerrURL' } });
    const jellyseerrKey = await prisma.setting.findUnique({ where: { key: 'JellyseerrKey' } });
    ...
    if (jellyseerrUrl?.value && jellyseerrKey?.value) {
      try {
        const jRes = await axios.get(`${jellyseerrUrl.value}/api/v1/request/count`, {
      ...
      try {
        ...
        if (d?.metrics) {
          ...
          if (d.metrics.temperature?.sensors) {
            ...
            const cpuSensor = d.metrics.temperature.sensors.find((s: any) => (s.name || '').toLowerCase().includes('cpu'));
            if (cpuSensor) unraidStats.cpuTemp = cpuSensor.current?.value || 0;
            const gpuSensor = d.metrics.temperature.sensors.find((s: any) => {
              const name = (s.name || '').toLowerCase();
              return name.includes('gpu') || name.includes('nvidia') || name.includes('amd');
            });
            if (gpuSensor) unraidStats.gpuTemp = gpuSensor.current?.value || 0;
          }
        }
      } catch (e) {

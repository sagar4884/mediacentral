...
const sendSyncNotification = async (message: string) => {
  const notify = await prisma.setting.findUnique({ where: { key: 'PushoverNotifySyncCompletion' } });
  ...
  push.send({ title: 'MediaCentral Sync', message }, (err: any) => {
...
router.post('/sync', async (req, res) => {
  try {
    ...
    if (service) {
      ...
      if (srv === 'radarr') {
        taskId = taskQueue.enqueue('Sync Radarr', (id, progress, checkCancelled) => syncService.syncRadarr(id, progress, checkCancelled), true);
      } else if (srv === 'sonarr') {
        ...
        taskId = taskQueue.enqueue('Sync Plex', async (id, progress, checkCancelled) => {
          progress(0);
          try {
            ...
            if (tokenSetting?.value) {
              // 1. Fetch Friends
              try {
                const res = await axios.get('https://plex.tv/api/v2/friends', {
              ...
              if (urlSetting?.value) {
                try {
                  ...
                  const libRes = await axios.get(`${baseUrl}/library/sections`, {
                    headers: { 'X-Plex-Token': tokenSetting.value, 'Accept': 'application/json' },
                    validateStatus: () => true
                  });
              ...
              const countUsers = await prisma.plexUser.count();
              if (countUsers === 0) {
                await prisma.plexUser.upsert({ where: { id: 'mock1' }, update: {}, create: { id: 'mock1', username: 'Uncle_Bob', warnings: 0 } });
                await prisma.plexUser.upsert({ where: { id: 'mock2' }, update: {}, create: { id: 'mock2', username: 'College_Roommate', warnings: 2 } });
                await prisma.plexUser.upsert({ where: { id: 'mock3' }, update: {}, create: { id: 'mock3', username: 'Cousin_Vinny', warnings: 3, banUntil: new Date(Date.now() + 86400000) } });
                
                await prisma.plexLibrary.upsert({ where: { id: 'lib1' }, update: {}, create: { id: 'lib1', name: 'Movies', type: 'movie' }});
                await prisma.plexLibrary.upsert({ where: { id: 'lib2' }, update: {}, create: { id: 'lib2', name: 'TV Shows', type: 'show' }});
              }
            } else {
        ...
        taskId = taskQueue.enqueue('Sync Jellyseerr', async (id, progress, checkCancelled) => {
          progress(0);
      ...
      // Global sync
      taskId = taskQueue.enqueue('Global Media Sync', async (id, progress, checkCancelled) => {
        progress(0);
...
async function verifyConnection(service: string, url: string, key: string, config?: Record<string, string>): Promise<'green' | 'yellow' | 'red'> {
  const needsUrl = !['tmdb', 'tvdb', 'gemini'].includes(service.toLowerCase());
  if (needsUrl && !url) return 'yellow'; // Missing config
  if (!key && service.toLowerCase() !== 'gemini') return 'yellow'; // Missing config
  ...
  try {
    ...
    if (service.toLowerCase() === 'tmdb') {
      try {
        ...
        fs.writeFileSync('api_test_error.log', `TMDB Test Error: ${e.message} ${JSON.stringify(e.response?.data)}\n`, { flag: 'a' });
    ...
    if (service.toLowerCase() === 'tvdb') {
      try {
        const loginRes = await axios.post('https://api4.thetvdb.com/v4/login', { apikey: key.trim() }, { timeout: 5000 });
        if (loginRes.data?.data?.token) return 'green';
        return 'red';
      } catch (e: any) {
        fs.writeFileSync('api_test_error.log', `TVDB Test Error: ${e.message} ${JSON.stringify(e.response?.data)}\n`, { flag: 'a' });
    ...
    if (service.toLowerCase() === 'unraid') {
      try {
        let uUrl = url;
        try { uUrl = new URL(url).origin; } catch(e) {}
        ...
        if (unraidRes.data && !unraidRes.data.errors) return 'green';
        return 'red';
      } catch (e: any) {
    ...
    if (service.toLowerCase() === 'gemini') {
      try {
        ...
        await axios.get(`https://generativelanguage.googleapis.com/v1beta/models/${modelToTest}?key=${key.trim()}`, { timeout: 5000 });
        return 'green';
      } catch (e: any) {
        return 'red';
      }
    ...
    if (service.toLowerCase() === 'pushover') {
      try {
        ...
        const pushRes = await axios.post('https://api.pushover.net/1/users/validate.json', params, { timeout: 5000 });
        if (pushRes.data?.status === 1) return 'green';
        return 'red';
      } catch (e: any) {
        return 'red';
      }
...
router.get('/status', async (req, res) => {
  try {
    ...
    const status: Record<string, 'green' | 'yellow' | 'red'> = {
      unraid: 'red',
      radarr: 'red',
      sonarr: 'red',
      jellyseerr: 'red',
    ...
    if (config.TMDBKey) status.tmdb = await verifyConnection('tmdb', '', config.TMDBKey, config);
    if (config.TVDBKey) status.tvdb = await verifyConnection('tvdb', '', config.TVDBKey, config);
    if (config.GeminiKey) status.gemini = await verifyConnection('gemini', '', config.GeminiKey, config);
    if (config.PushoverAppToken && config.PushoverUserKey) status.pushover = await verifyConnection('pushover', config.PushoverUserKey, config.PushoverAppToken, config);
...
router.post('/test', async (req, res) => {
  try {
    ...
    if (service.toLowerCase() === 'gemini') {
      if (!key) return res.status(400).json({ success: false, message: 'Gemini API Key is required' });
    ...
    if (!key && service.toLowerCase() !== 'gemini') return res.status(400).json({ success: false, message: 'API Key is required' });
    ...
    if (service.toLowerCase() === 'tmdb') {
      try {
        ...
        const tmdbHeaders = isBearer ? { Authorization: `Bearer ${apiKey}` } : {};
        const tmdbParams = isBearer ? {} : { api_key: apiKey };
        await axios.get('https://api.themoviedb.org/3/configuration', { headers: tmdbHeaders, params: tmdbParams, timeout: 5000 });
    ...
    if (service.toLowerCase() === 'tvdb') {
      try {
        const loginRes = await axios.post('https://api4.thetvdb.com/v4/login', { apikey: key.trim() }, { timeout: 5000 });
        if (loginRes.data?.data?.token) return res.json({ success: true, message: 'Successfully authenticated with TVDB' });
    ...
    if (service.toLowerCase() === 'pushover') {
      try {
        ...
        const pushRes = await axios.post('https://api.pushover.net/1/messages.json', params, { timeout: 5000 });
        if (pushRes.data?.status === 1) return res.json({ success: true, message: 'Test notification sent via Pushover!' });

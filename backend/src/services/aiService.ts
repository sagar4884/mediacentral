...
export class AIService {
  ...
  async getRules(source: string): Promise<string> {
    return await this.getSetting(`${source}AIRules`, "1. Keep movies with high IMDB scores.\n2. Delete reality TV.");
  }

  async saveRules(source: string, rules: string): Promise<void> {
  ...
  async curateMedia(source: string, reportProgress?: (progress: number) => void, checkCancelled?: () => boolean, selectedIds?: string[]): Promise<number> {
    ...
    const whereClause: any = { source, keepStatus: 'waiting' };
    ...
    if (waitingMedia.length === 0) return 0;
  ...
  async updateRules(source: string): Promise<string> {
    const apiKey = await this.getSetting('GeminiKey');
    if (!apiKey) throw new Error("Gemini API Key not set");
    ...
    const history = await prisma.mediaCache.findMany({
      where: { 
        source,
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        await prisma.setting.delete({ where: { key: `${source}AIPendingExplanation` } }).catch(() => {});

...
const sendPlexNotification = async (message) => {
    const notify = await index_1.prisma.setting.findUnique({ where: { key: 'PushoverNotifyPlexBan' } });
...
router.post('/users/:id/role', async (req, res) => {
    try {
        ...
        const user = await index_1.prisma.plexUser.findUnique({ where: { id } });
        if (!user)
...
router.get('/groups', async (req, res) => {
    try {
        const groups = await index_1.prisma.plexGroup.findMany({
...
router.post('/groups', async (req, res) => {
    try {
        ...
        const group = await index_1.prisma.plexGroup.create({
            data: {
                name,
                libraries: { connect: libraryIds.map((id) => ({ id })) }
            },
            include: { libraries: true }
        });
        res.json(group);
...
router.delete('/groups/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await index_1.prisma.plexGroup.delete({ where: { id } });
        res.json({ success: true });
    }
    catch (error) {
...
router.get('/roles', async (req, res) => {
    try {
        const roles = await index_1.prisma.plexRole.findMany({
...
router.post('/roles', async (req, res) => {
    try {
        ...
        const role = await index_1.prisma.plexRole.create({
            data: {
                name,
...
router.delete('/roles/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await index_1.prisma.plexRole.delete({ where: { id } });
        res.json({ success: true });
    }
    catch (error) {
...
router.get('/libraries', async (req, res) => {
    try {
        const libraries = await index_1.prisma.plexLibrary.findMany();
        res.json(libraries);
    }
    catch (error) {
...
router.post('/push', async (req, res) => {
    try {
        const result = await plexService_1.plexService.pushToPlex();
        res.json(result);
    }
    catch (error) {
...
router.post('/revoke/:username', async (req, res) => {
    try {
        ...
        let bannedRole = await index_1.prisma.plexRole.findUnique({ where: { name: 'Temporarily Banned' } });
        if (!bannedRole) {
            bannedRole = await index_1.prisma.plexRole.create({ data: { name: 'Temporarily Banned' } });
        }
        const user = await index_1.prisma.plexUser.findUnique({ where: { id: username } });
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        ...
        const latestViolation = await index_1.prisma.plexViolation.findFirst({
            where: { userId: username },
            orderBy: { createdAt: 'desc' }
        });
        if (latestViolation) {
            await index_1.prisma.plexViolation.update({
        ...
        tautulliMonitor_1.tautulliMonitor.terminateUserStreams(username, `Account access revoked for ${days} days by administrator.`).catch(e => console.error(e));
        // Push only this user to Plex to instantly enact the ban
        plexService_1.plexService.pushToPlex(username).catch(e => console.error(e));
...
router.post('/unban/:username', async (req, res) => {
    try {
        ...
        plexService_1.plexService.pushToPlex(user.id).catch(e => console.error("Auto push failed on unban:", e));
...
router.get('/activity', async (req, res) => {
    try {
        ...
        const response = await axios_1.default.get(`${urlSetting.value}/api/v2`, {
...
router.post('/activity/terminate', async (req, res) => {
    try {
        ...
        await axios_1.default.get(`${urlSetting.value}/api/v2`, {
            params: {
...
router.get('/image', async (req, res) => {
    try {
        ...
        if (!thumb)
            return res.status(400).send('No thumb provided');
        const urlSetting = await index_1.prisma.setting.findUnique({ where: { key: 'TautulliURL' } });
        const keySetting = await index_1.prisma.setting.findUnique({ where: { key: 'TautulliKey' } });
        if (!urlSetting?.value || !keySetting?.value) {
...
router.get('/violations', async (req, res) => {
    try {
        const violations = await index_1.prisma.plexViolation.findMany({
            include: { user: true },
            orderBy: { createdAt: 'desc' }
        });
        res.json(violations);
...
router.delete('/violations/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await index_1.prisma.plexViolation.delete({ where: { id } });
        res.json({ success: true });
    }
    catch (error) {
